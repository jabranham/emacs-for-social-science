...
** Mac
   Macs do not come with a spell checking program installed that Emacs can use. You'll need to install one - either hunspell or aspell. The recommended way is with [[http://brew.sh/][homebrew]]:
